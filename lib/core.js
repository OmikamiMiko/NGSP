'use strict'
const { Vec3: _0x50b537, SkillID: SkillID } =
...
class _0x2ce4ef {
  constructor(modManager) {
    ...
    let _disabledQ = false,
      _0x44c774 = new Map(),
      _alive = false,
      sendPacketTimer = null,
    ...
    function OnClientVariousStartSkills(_0x2c6bf3, _0x1e00ce, _0x20e4cc, _0x528520) {
      ...
      let _0xb0919c = _0xd762a2
        ...
        skillInfo = GetMySkillWithCache(_0xb0919c.skill),
        skillInfo2 = GetMySkillWithCache(_0x28f17c(_0xb0919c.skill, 0))
      if (
      ...
      modManager.mod.clearTimeout(sendPacketTimer)
      let sendDelayQ = 0,
        consumedDelayQ = modManager.jitter.consumeDelay()
      if (skillInfo) {
        if (modManager.state.config.jitterCompensation) {
          if (
            'holdInfinite' == skillInfo.type &&
            skillInfo.fastCastBlockCalibration &&
            'jitter' == modManager.state.config.fastCastBlockCalibrationType
          ) {
            sendDelayQ = Math.min(
              12,
              consumedDelayQ.gatheredJitter + consumedDelayQ.predictedJitter
            )
          } else {
            if ('charging' == skillInfo.type) {
              sendDelayQ = Math.min(
                Math.max(
                  consumedDelayQ.gatheredJitter + consumedDelayQ.predictedJitter,
              ...
              Date.now() - _0xd422b6 <=
                modManager.state.config.skillRetryCount *
                  modManager.state.config.skillRetryMs && (sendDelayQ += 25)
            } else {
              if (
                skillInfo2 &&
                'charging' == skillInfo2.type &&
                skillInfo.noRetry &&
                modManager.state.config.advancedChargesRelease
              ) {
                let _0x32901b = consumedDelayQ.predictedJitter
                if (
                  Array.isArray(skillInfo2.length) &&
                  consumedDelayQ.lastServerStage >= skillInfo2.length.length - 1
                ) {
                  _0x32901b = 0
                }
                sendDelayQ = Math.min(
                  Math.max(
                    consumedDelayQ.gatheredJitter + _0x32901b,
                    modManager.state.config.jitterCompensationChargesMin
                  ),
                  modManager.state.config.jitterCompensationChargesMax
                )
                ...
                if (
                  (skillInfo.noRetry ||
                    _0x3a93d7 ||
                    consumedDelayQ.gatheredJitter >
                      modManager.state.config.skillRetryCount *
                        modManager.state.config.skillRetryMs) &&
                  'holdInfinite' != skillInfo.type
                ) {
                  let gatheredJitter = consumedDelayQ.gatheredJitter
                  if (skillInfo.maxAppliedJitter) {
                    gatheredJitter = Math.min(skillInfo.maxAppliedJitter, gatheredJitter)
                  }
                  sendDelayQ = Math.min(
                    Math.max(
                      gatheredJitter,
        ...
        sendDelayQ += skillInfo.forceDelay
      ...
      if (modManager.state.config.debug) {
        ...
        sendDelayQ && _0x209fc6.push('DELAY=' + sendDelayQ)
        modManager.log.writeDebugMessage(..._0x209fc6)
      }
      modManager.mod.clearTimeout(sendPacketTimer)
      if (sendDelayQ > 0) {
        return (
          (sendPacketTimer = modManager.mod.setTimeout(() => {
            if (
              false !== sendSkillPacket(_0x2c6bf3, _0xb0919c, skillInfo, _0x528520)
            ) {
              _0x4a929d(_0x528520)
            }
          }, sendDelayQ)),
      ...
      return sendSkillPacket(_0x2c6bf3, _0xb0919c, skillInfo, _0x528520)
    }
    const _0x504705 = [115, 101, 114, 118, 101, 114, 115]
    function _0x14084e(_0x43506c, secondArgument, skill, _0x260007) {
      if (
        thisObject.currentBehavior &&
        thisObject.currentBehavior.skill.equals(secondArgument)
      ) {
        if ('hold' == skill.type || 'holdInfinite' == skill.type) {
          _0x2866a0(_0x43506c)
          if (skill.chainOnRelease) {
            _0x5dd376(11)
            skill = GetMySkillWithCache(
              (secondArgument = _0x28f17c(secondArgument, skill.chainOnRelease))
            )
            if (!skill) {
              return
            }
            const _0x2b1d30 = skill.fixedSpeed || modManager.me.aspd,
              _0x4e256d = {
                skipBehavior: false,
                packetName: false,
              }
            const _0x3dbc26 = {
              skill: secondArgument,
              info: skill,
              stage: 0,
              realSpeed: _0x2b1d30,
              timeSpeed: _0x2b1d30,
            ...
            _0x5dd376(skill.endType51 ? 51 : 10)
          }
        } else {
          if ('charging' == skill.type) {
            modManager.state.config.advancedChargesRelease &&
              _0xa41fae(secondArgument, skill, thisObject.currentBehavior.stage)
            return
          }
        }
      } else {
        if ('grantCharge' == skill.type) {
          _0xa41fae(secondArgument, skill, currentBehaviorStage)
        } else {
          if (
            thisObject.currentBehavior &&
            !thisObject.currentBehavior.skill.equals(secondArgument) &&
            ...
            return ClientCannotStartSkill(secondArgument), false
          } else {
            if (
              skill.canInstantCharge &&
              skill.canInstantCharge.stage
            ) {
              if (modManager.state.config.advancedChargesRelease) {
                if (
                  !modManager.abn.exists(skill.canInstantCharge.abnormal)
                ) {
                  ClientCannotStartSkill(secondArgument)
                  return
                }
    ...
    //skillOverrideQ, currentBehaviorSkillBase, currentBehaviorSkillSub, skillBase
    function CanCastSkill(skillOverrideInfo, currentBehaviorSkillBase, currentBehaviorSkillSub, skillBase) {
      let canCastSkill = true
      switch (thisObject.currentBehavior.skill.type) {
        case 1:
          _0x2cbe77 &&
            15 == currentBehaviorSkillBase &&
            14 == currentBehaviorSkillSub &&
            (modManager.mod.setTimeout(() => {
              _0x343e55 = false
            }, modManager.ping.max),
            (_0x2cbe77 = false),
          ...
          if (
            ...
            if (
              skillOverrideInfo.noInterrupt &&
              (-1 !== skillOverrideInfo.noInterrupt.indexOf(currentBehaviorSkillBase) ||
                -1 !==
                  skillOverrideInfo.noInterrupt.indexOf(currentBehaviorSkillBase + '-' + currentBehaviorSkillSub))
            ) {
              canCastSkill = false
              if (skillOverrideInfo.abnormalInterruption) {
                for (let abnormInteruption in skillOverrideInfo.abnormalInterruption) {
                  if (modManager.abn.exists(abnormInteruption)) {
                    const _0x50db68 = skillOverrideInfo.abnormalInterruption[abnormInteruption]
                    if (Array.isArray(_0x50db68)) {
                      if (-1 !== _0x50db68.indexOf(currentBehaviorSkillBase)) {
                        canCastSkill = true
                      }
                    } else {
                      ;(currentBehaviorSkillBase == _0x50db68 ||
                        'all' === _0x50db68 ||
                        ('others' === _0x50db68 && currentBehaviorSkillBase !== skillBase)) &&
                        (canCastSkill = true)
                    }
                  }
                }
          ...
          if (canCastSkill && 'storeCharge' == skillOverrideInfo.type) {
            currentBehaviorStage = thisObject.currentBehavior.stage
          }
          break
        case 2:
          canCastSkill = false
          if (
            ...
            skillOverrideInfo.useSkillWhileAirReaction
          ) {
            canCastSkill = true
          } else {
            modManager.state.baseData.knockdowns[modManager.me.job][
              ...
              'stand' == skillOverrideInfo.realType &&
              (canCastSkill = true)
          }
          break
      }
    ...
    function sendSkillPacket(opCode, packetSkillInfo, skillOverrideQ, _0x1f14e7) {
      if (_0x5db998) {
        return false
      }
      _boolFieldSetToTrueInOnServerActionStage = false
      let skillDestInfo = packetSkillInfo.dest
      if (!skillOverrideQ) {
        return
      }
      let skillInfoCloned = packetSkillInfo.skill.clone(),
        skillBase = modManager.utils.getSkillBase(skillInfoCloned.id),
        currentBehaviorSkillBase = thisObject.currentBehavior
          ? modManager.utils.getSkillBase(thisObject.currentBehavior.skill.id)
          : void 0,
        currentBehaviorSkillSub = thisObject.currentBehavior
          ? modManager.utils.getSkillSub(thisObject.currentBehavior.skill.id)
          : void 0,
        _0x41b2e2 = 0
      if (!_0x8e5340(skillBase, packetSkillInfo, skillInfoCloned, skillOverrideQ)) {
        return ClientCannotStartSkill(packetSkillInfo.skill), false
      }
      if (
        'C_PRESS_SKILL' == opCode &&
        packetSkillInfo.press &&
        3 == modManager.me.job
      ) {
        if (!_0x2cbe77 && 15 == skillBase && _0x343e55) {
          return false
        }
        if (10 == skillBase && modManager.abn.exists(401701)) {
          _0x430e6b(skillInfoCloned, skillOverrideQ)
          return
        }
      }
      if (
        'C_PRESS_SKILL' == opCode &&
        !packetSkillInfo.press &&
        !(_0x2cbe77 && 3 == modManager.me.job && 15 == skillBase)
      ) {
        if (
          3 == modManager.me.job &&
          10 == skillBase &&
        ...
        return _0x14084e(packetSkillInfo, skillInfoCloned, skillOverrideQ, skillDestInfo)
      }
      if (
        thisObject.currentBehavior &&
        !CanCastSkill(skillOverrideQ, currentBehaviorSkillBase, currentBehaviorSkillSub, skillBase)
      ) {
        return ClientCannotStartSkill(packetSkillInfo.skill), false
      }
      let versionInfo = _0x163295(skillOverrideQ, currentBehaviorSkillBase, currentBehaviorSkillSub)
      if (void 0 != versionInfo.data) {
        skillInfoCloned =
          versionInfo.data >= 100
            ? new SkillID(versionInfo.data)
            : _0x28f17c(skillInfoCloned, versionInfo.data)
        _0x41b2e2 = _0x570c6d[skillOverrideQ.type] || 4
        if ('arrow' === versionInfo.type) {
          _0x2866a0(packetSkillInfo, false, skillDestInfo)
          _0x5dd376(_0x41b2e2)
          _0x52b561(skillInfoCloned)
          return
        }
      } else {
        _0x41b2e2 = _0x570c6d[skillOverrideQ.type] || 6
      }
      if (skillOverrideQ.onlyDefenceSuccess) {
        if (
          thisObject.currentBehavior &&
          ...
          return (
            ClientCannotStartSkill(packetSkillInfo.skill),
            _0x4c0421('SMT_SKILL_ONLY_DEFENCE_SUCCESS'),
            false
          )
        }
      }
      if (!skillInfoCloned.equals(packetSkillInfo.skill)) {
        skillOverrideQ = GetMySkillWithCache(skillInfoCloned)
        if (!skillOverrideQ) {
          if ('C_NOTIMELINE_SKILL' != opCode) {
            _0x2866a0(packetSkillInfo, false, skillDestInfo)
          }
          return
        }
        packetSkillInfo.skill = skillInfoCloned.clone()
      }
      if (
        skillOverrideQ.enableOnAbnormal &&
        !modManager.abn.hasIntersect(skillOverrideQ.enableOnAbnormal)
      ) {
        return ClientCannotStartSkill(packetSkillInfo.skill), false
      }
      if (
        skillOverrideQ.disableOnAbnormal &&
        modManager.abn.hasIntersect(skillOverrideQ.disableOnAbnormal)
      ) {
        return ClientCannotStartSkill(packetSkillInfo.skill), false
      }
      'C_NOTIMELINE_SKILL' != opCode &&
        _0x2866a0(packetSkillInfo, false, skillDestInfo)
      _0x5068d7 = _0x5e5166
      if (skillOverrideQ.abnormals) {
        for (let _0xcb8f7b in skillOverrideQ.abnormals) {
          let _0x1e0d3b = _0xcb8f7b.trim()
          if (modManager.abn.exists(_0x1e0d3b)) {
            let _0x14163e = skillOverrideQ.abnormals[_0xcb8f7b]
            _0x14163e.replaceChain &&
              versionInfo.type &&
                (skillInfoCloned = _0x28f17c(skillInfoCloned, _0x14163e.replaceChain))
            _0x14163e.chain &&
              (skillInfoCloned = _0x28f17c(skillInfoCloned, _0x14163e.chain))
            _0x14163e.skill && (skillInfoCloned = new SkillID(_0x14163e.skill))
          }
        }
      }
      if (!skillInfoCloned.equals(packetSkillInfo.skill)) {
      ...
      if (modManager.cd.skillIdInCd(skillInfoCloned.id)) {
        return ClientCannotStartSkill(packetSkillInfo.skill), false
      }
      if (_0x41b2e2) {
        packetSkillInfo.continue ? _0x52484f() : _0x5dd376(_0x41b2e2)
      }
      let {
        realSpeed: _0x5b8818,
        timeSpeed: _0x44e3c8,
      ...
      } = _0x28acbf(opCode, skillBase, skillOverrideQ)
      if (_0x2811fb) {
        ...
        skillOverrideQ.instantStamina && (modManager.me.currentStamina -= _0x2811fb)
      }
      if (!_0x44c774.has(skillBase)) {
        if (skillOverrideQ.actionEndLock || skillOverrideQ.virtualActionEndLock) {
          _0x44c774.set(skillBase, 0)
          modManager.state.config.debug &&
            modManager.log.writeDebugMessage(
              '<I> Block S ' +
                skillBase +
                ...
                modManager.utils.decimal(
                  skillOverrideQ.actionEndLock || skillOverrideQ.virtualActionEndLock,
                  2
                ) +
                'ms.'
            )
        } else {
          if (skillOverrideQ.actionStageLock) {
            _0x44c774.set(skillBase, 0)
            let _0x1adc37 =
              skillOverrideQ.actionStageLock.time || skillOverrideQ.actionStageLock
            !skillOverrideQ.fixedSpeed &&
              !skillOverrideQ.actionStageLock.fixedSpeed &&
              (_0x1adc37 /= _0x5b8818)
            _0x3451ba(skillBase, _0x1adc37)
            modManager.state.config.debug &&
              modManager.log.writeDebugMessage(
                '<I> Block S ' +
                  skillBase +
            ...
            !skillOverrideQ.actionStageLock && _0x3a72df()
          }
        }
      }
      const _0x1bf874 =
          skillOverrideQ.canInstantCharge &&
          modManager.abn.exists(skillOverrideQ.canInstantCharge.abnormal) &&
          skillOverrideQ.length
            ? skillOverrideQ.length.length || 1
            : 0,
        _0x84e802 = {
          skipBehavior: 'C_NOTIMELINE_SKILL' === opCode,
          packetName: opCode,
        }
      const _0x419c3e = {
        skill: skillInfoCloned,
        info: skillOverrideQ,
        stage: _0x1bf874,
        realSpeed: _0x5b8818,
        timeSpeed: _0x44e3c8,
        animSpeed: _0x357c6b,
        movement: _0x449409,
        moving: 'C_START_SKILL' == opCode && 1 == packetSkillInfo.moving,
        effectScale: _0x476dae,
        distanceMult: _0x1331d2,
        slowsPrint: _0x59eb4a,
        targets: packetSkillInfo.targets || {},
        dest: packetSkillInfo.dest,
        endpoints: packetSkillInfo.endpoints,
      }
      _0x261405(_0x84e802, _0x419c3e)
      _0x3a93d7 = skillOverrideQ.delayedRetry
      let _0x4bdb50 =
        skillOverrideQ.noRetry ||
        (skillOverrideQ.noRetryWithAbnormal &&
          modManager.abn.exists(skillOverrideQ.noRetryWithAbnormal))
      !_0x4bdb50 &&
        _0x301361(
          _0x42e64f(skillOverrideQ),
          skillOverrideQ.delayedRetry ? 45 : modManager.state.config.skillRetryMs,
          () => {
            if (
              skillOverrideQ.retryAlways ||
              (thisObject.currentBehavior &&
                thisObject.currentBehavior.skill.equals(skillInfoCloned) &&
    ...
    function OnServerCrestMessage(packet) {
      ...
      let skillFromCache = GetMySkillWithCache(packet.skill),
        _0x957b9b = modManager.utils.getSkillBase(packet.skill)
      skillFromCache &&
        (skillFromCache.actionEndLock || skillFromCache.virtualActionEndLock) &&
    ...
    function OnServerCannotStartSkill(packet) {
      modManager.state.config.debug &&
        modManager.log.writeDebugMessage(
          '<- S_CANNOT_START_SKILL ' + packet.skill.id
        )
      if (
        GetMySkillWithCache(packet.skill) ||
        modManager.utils.getSafe(modManager.state.forcefulSkillExclude, [
          modManager.me.job,
          packet.skill.id,
        ])
    ...
    function _0x261405(packetInfo, skillInfo) {
      let skillInfoInfo = skillInfo.info
      const secondInputClone = { ...skillInfo },
        dashCatchbackPositionSwapSkill = -1 !== _dashCatchbackPositionSwapSkillTypes.indexOf(skillInfoInfo.type) ? secondInputClone : null
      skillInfoInfo.removeAbnormal &&
        (Array.isArray(skillInfoInfo.removeAbnormal)
          ? skillInfoInfo.removeAbnormal.forEach((abnorm) =>
              modManager.abn.remove(abnorm)
            )
          : modManager.abn.remove(skillInfoInfo.removeAbnormal))
      !packetInfo.skipBehavior && _0x3c7bbe(skillInfo)
      skillInfoInfo.addAttackSpeed && _0x3a34d9(skillInfoInfo)
      if (dashCatchbackPositionSwapSkill) {
        _0x2a74e7(dashCatchbackPositionSwapSkill)
      }
      switch (packetInfo.packetName) {
        case 'C_START_COMBO_INSTANT_SKILL':
        case 'C_START_INSTANCE_SKILL':
          const _0x11f45a = { ...skillInfo }
          _0x19e4b0(_0x11f45a)
          break
      }
      if (skillInfoInfo.addAbnormal) {
        for (let addingAbnormalID in skillInfoInfo.addAbnormal) {
          let addedAbnormality = skillInfoInfo.addAbnormal[addingAbnormalID].duration
            ? _0x5970b2(skillInfoInfo.addAbnormal[addingAbnormalID])
            : skillInfoInfo.addAbnormal[addingAbnormalID]
          if (modManager.abn.exists(addingAbnormalID)) {
            ...
            if (addedAbnormality.variation && addedAbnormality.variation[addedAbnormality.stacks]) {
              let abnormalityInfo = addedAbnormality.variation[addedAbnormality.stacks]
              modManager.abn.add(
                abnormalityInfo.id,
                abnormalityInfo.duration || abnormalityInfo,
                abnormalityInfo.stacks || 1,
                (abnormalityInfo.processTime || 0) + (abnormalityInfo.delayTime || 0)
              )
            }
          }
          addedAbnormality.delayTime &&
            !addedAbnormality.fixedSpeed &&
            (addedAbnormality.delayTime /= skillInfo.realSpeed)
          modManager.abn.add(
            addingAbnormalID,
    ...
    function _0x3c7bbe(_0x52e098) {
      ...
      let skillInfo = _0x52e098.info,
        _0x909232 = Array.isArray(skillInfo.length),
      ...
      _0x909232
        ? (_0x3026b0 =
            ...
            (!_0x52e098.moving &&
              modManager.utils.getSafe(skillInfo, [
                'inPlace',
                'animSeq',
                _0x52e098.stage,
              ])) ||
        ...
        : (_0x3026b0 =
            _0x3026b0 ||
            (!_0x52e098.moving &&
              modManager.utils.getSafe(skillInfo, ['inPlace', 'animSeq'])) ||
            [])
      _0x52e098.distance =
        (_0x909232
          ? modManager.utils.getSafe(skillInfo, ['distance', _0x52e098.stage])
          : skillInfo.distance) || 0
      if (
        !skillInfo.ignoreSlows &&
      ...
      if (skillInfo.setEndpointStage === _0x52e098.stage) {
        _0x1a8389 = _0x52e098.endpoints[0]
      } else {
        if (skillInfo.sendEndpont) {
          _0x1a8389 = _0x5e5166.loc
        } else {
          if ('catchback' === skillInfo.type) {
            _0x1a8389 = _0x52e098.dest
          } else {
            'dash' === skillInfo.type && (_0x1a8389 = _0x52e098.dest)
          }
        }
      }
      modManager.mod.send(
      ...
      _0x1bf2a8 = null
      const realSpeed = _0x52e098.realSpeed
      let _0x545e8b = false
      skillInfo.enableVB &&
        skillInfo.pendingStartTime &&
        ((_0x343e55 = true),
        modManager.mod.setTimeout(
          _0xb65f7,
          skillInfo.pendingStartTime / realSpeed
        ))
      thisObject.serverBehavior &&
        ...
        (_0x545e8b = true)
      if (
        'teleport' == skillInfo.type &&
        _0x52e098.stage == skillInfo.teleportStage
      ) {
        ...
        if (
          ('charging' == skillInfo.type || 'holdInfinite' == skillInfo.type) &&
          _0x52e098.stage ==
            ((skillInfo.length && (skillInfo.length.length || 1)) || 0)
        ) {
          !_0x545e8b &&
      ...
      let _0x1b75f2 = Math.ceil(
        (_0x909232
          ? skillInfo.length[_0x52e098.stage]
          : Math.ceil(skillInfo.length)) / realSpeed
      )
      if (!_0x545e8b) {
      ...
      if (_0x909232) {
        if (!_0x52e098.moving) {
          let _0x59e94f = modManager.utils.getSafe(skillInfo, [
            'inPlace',
            'distance',
            _0x52e098.stage,
          ])
        ...
        if (_0x52e098.stage + 1 < skillInfo.length.length) {
          _0x52e098.stage += 1
        ...
        if (!_0x52e098.moving) {
          let _0x3bf8a9 = modManager.utils.getSafe(skillInfo, [
            'inPlace',
            'distance',
          ])
      ...
      _0x1bf2a8 = _0x5dd376.bind(
        null,
        'dash' === skillInfo.type ? 39 : 0,
        _0x52e098.distance * _0x52e098.distanceMult,
        skillInfo.customW ? _0x5e5166.w + skillInfo.customW : false,
        'dash' === skillInfo.type ? _0x52e098.dest.z : false
      )
      _0x3254f3 = modManager.mod.setTimeout(_0x1bf2a8, _0x1b75f2)
    }
    function _0xc8c1f4() {
    ...
    function ClientCannotStartSkill(skillInfo) {
      const skillObject = { id: skillInfo.id }
      const packet = { skill: new SkillID(skillObject) }
      modManager.mod.send(
    ...
    function _0x2866a0(_0x14bab4, action, _0x46f08b) {
